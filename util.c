...
int build(char * const TargetName, target_t * const target, int const nTargetCount) {
	int tIndex = find_target(TargetName, target, nTargetCount);
	int dIndex;

	target[tIndex].Status = 2;

	if(tIndex >= 0) { // when target index is greater than or equal to 0
		target[tIndex].DependencyCount;

		for(dIndex = 0; dIndex < target[tIndex].DependencyCount; dIndex++) {
			char* depName = target[tIndex].DependencyNames[dIndex]; // set dependency name

			build(depName, target, nTargetCount);

			if(does_file_exist(depName) == 0 && does_file_exist(TargetName) == 0) {

				int time = compare_modification_time(TargetName, depName);

				if(time == -1) {
				//	printf(" %s or %s does not exist\n", TargetName, depName);
				} else if(time == 0) {
				//	printf(" %s and %s updated at the same time\n", TargetName, depName);
				} else if(time == 1) {
				//	printf(" %s updated later than %s\n", TargetName, depName);
				} else if(time == 2) {
					//printf(" Target %s needs to be rebuilt\n", TargetName);
					int st = build(target[tIndex].DependencyNames[dIndex], target, nTargetCount);
					if(st = UNFINISHED) {
				...
				//printf(" %s or %s does not exist, set status to 2\n", TargetName, depName);
				target[tIndex].Status = 2;
			}
		} // execute

		if(target[tIndex].Status != 2) {
			printf(" Target %s status = %d. No need to be built and returning.\n", TargetName, target[tIndex].Status);
			return -1;
		}
	} else if(does_file_exist(TargetName) == -1) {
		fprintf(stderr, "Error: '%s' is not a target or a file.\n", TargetName);
	} else {
		//printf(" %s is a file but not a target, returning.\n", TargetName);
		return -1;
	}

		target[tIndex].Status = 0; // set the status

		pid_t myPID = fork();

		if(myPID == -1) { // child verson -- needs to execute Command
			perror("Faled to fork\n");
			exit(-1);
		} else if(myPID == 0) {

			//fprintf(stdout, "pid = 0 \n");
			//fprintf(stdout, "fork command: %s\n", target[tIndex].Command);
			execl("/bin/sh","sh","-c",target[tIndex].Command,(char *)0);
			execvp(target[tIndex].tokens[0], target[tIndex].tokens);
			printf("execpv() failed with error %s\n", strerror(errno));
			exit(-1);
		} else {
			int st;
			wait(&st);
			...
			if(WEXITSTATUS(st) != 0) {
				fprintf(stderr, "make4061: *** [%s] Error %d\n", TargetName, WEXITSTATUS(st));
				exit(-1);
			}
		}

		//printf("Target %s is done\n", TargetName);
